...
//! explicit continuations instead of nested return-value plumbing.
use cel::cps::{run, Pipeline, Step};
...
/// into the next by name. `Pipeline` runs them in order and stops at the
/// first error.
fn pipeline_demo() {
...
/// whether the chain is three steps or three hundred thousand.
fn branching_demo() {
    ...
    let step = Step::cont(&is_eligible, |eligible, _ctx| {
        if eligible == Ok(true.into()) {
